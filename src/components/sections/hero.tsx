  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap
        ...
        .fromTo(".hero-line", { scaleX: 0 }, { scaleX: 1, duration: 0.5 })
        .from(".hero-word", { yPercent: 110, duration: 0.85, stagger: 0.08 }, "-=0.15")
    ...
    <section ref={rootRef} className="overflow-hidden border-b border-border">
      ...
      <Container className="grid min-h-[88vh] items-center gap-16 py-24 lg:grid-cols-[55%_45%]">
        <div className="flex flex-col gap-8">
          <span className="hero-line block h-px w-[60px] origin-left bg-accent" />
          ...
          <div className="hero-cta">
            <Button href="/booking" variant="outline" size="lg" className="w-fit border-accent-border text-accent hover:text-accent-hover">
              Записатися
            </Button>
          </div>
        </div>
        ...
        <div className="hero-visual relative hidden aspect-square items-center justify-center lg:flex" aria-hidden>
          <div
            ...
            style={{
              background:
                "radial-gradient(circle, color-mix(in srgb, var(--color-accent) 38%, transparent), transparent 70%)",
            }}
          />
          <div
            className="absolute h-64 w-64 rounded-full border border-accent-border"
